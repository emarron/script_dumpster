...
for file in $(find $1 -name "*.tga" -type f)
    ...
    then
        ...
        then
            ...
            ./cuttlefish.exe -i $file -j -m 12 -f BC7  -Q highest -o "dds/${output%.tga}.dds" --create-dir
        else 
            echo "opacity (RGBA) is false, d>=8192, saving as BC3, 12mipmaps" 
            ./cuttlefish.exe -i $file -j -m 12 -f BC3 -Q normal -o "dds/${output%.tga}.dds" --create-dir
        fi
    else
        if [ $opacity == 'True' ]
        then
            echo "opacity (RGB) is true, d<8192, saving as BC7" 
            ./cuttlefish.exe -i $file -j -m -f BC7  -Q highest -o "dds/${output%.tga}.dds" --create-dir
        else
            if [ $width -le 512 ] || [ $height -le 512 ]
            then
                echo "opacity (RGBA) is false, d<=512, saving as BGRA" 
                ./cuttlefish.exe -i $file -j -m -f B8G8R8A8 -o "dds/${output%.tga}.dds" --create-dir
            else
                echo "opacity (RGBA) is false, 512<d<8192, saving as BC3" 
                ./cuttlefish.exe -i $file -j -m -f BC3 -Q normal -o "dds/${output%.tga}.dds" --create-dir
            fi               
        fi
    fi
done   
