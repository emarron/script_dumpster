...
for file in $(find $1 -name "*.*" -type f)
    ...
    then
        ...
        then
            ...
            ./cuttlefish.exe -i $file -j -m 12 -f BC3 -Q normal -o "dds/${output%.*}.dds" --create-dir
        fi
    else
        if [ $opacity == 'True' ]
        then
            echo "opacity (RGB) is true, d<8192, saving as BC7"
            # THIS WAS BROKEN RERUN THIS ON DIFFUSE, NORMALS
            ...
            then
                ...
                ./cuttlefish.exe -i $file -j -m -f B8G8R8A8 -o "dds/${output%.*}.dds" --create-dir
            else
                echo "opacity (RGBA) is false, 512<d<8192, saving as BC3" 
                ./cuttlefish.exe -i $file -j -m -f BC3 -Q normal -o "dds/${output%.*}.dds" --create-dir
            fi               
        fi
    fi
done   
